...
class Trainer(object):
    def __init__(self, model, lrate, min_lrate, wdecay, betas, eps, amsgrad, clip, steps_per_epoch, num_epochs, total_iters, 
                ...
                finetune_modules=None, patience=0, min_delta=0.0):
        ...
        self.min_delta = min_delta or 0.0
    ...
    def _is_improvement(self, current_result, best_result):
        if self.evaluate_metric in ['rmse','mse','mae']:
            return current_result < (best_result - self.min_delta)
        return current_result > (best_result + self.min_delta)

    def _should_stop_early(self, num_bad_evals):
        return self.patience > 0 and num_bad_evals >= self.patience
        ...
        with output(initial_len=11, interval=0) as output_lines:
            for epoch in range(1, self.num_epochs+1):
                ...
                if epoch % evaluate_epoch == 0 and val_loader is not None:
                    ...
                    better_than_previous = self._is_improvement(val_result[self.evaluate_metric], best_result)
                ...
                if epoch % evaluate_epoch == 0  and val_loader is None:
                    ...
                    better_than_previous = self._is_improvement(test_result[self.evaluate_metric], best_result)
                ...
                if epoch % evaluate_epoch == 0 and val_loader is not None and self._should_stop_early(early_stop_counter):
                    early_stop_msg = f'Early stopping at epoch {epoch:03d} with best {self.evaluate_metric}: {best_result:.4f}'
            ...
            while iter_num < self.total_iters:
                for data in train_loader:
                    ...
                    if (iter_num % evaluate_step == 0) or (iter_num  == self.total_iters):
                        ...
                        if val_loader is not None and self._should_stop_early(early_stop_counter):
                            early_stop_msg = f'Early stopping at step {iter_num} with best {self.evaluate_metric}: {best_result:.4f}'
                            print(early_stop_msg)
                            with open(self.result_path +'/full_result-{}.txt'.format(self.runid),'a+') as f:
                                f.write(early_stop_msg + '\n')
                            stop_training = True
